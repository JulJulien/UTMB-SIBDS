...
--------------------
Meeting
Can each serum level predict whether or not someone has chagas disease?
- logistic regression (only two outcomes)
- overfitting (ROC and AUC)
- two most important predictors log reg 
*logistic regression, ROC, AUC*
--------------------
``` {r logreg}
log.reg <- glm(chag ~ HnRNPA1_Serum, data = data, family = "binomial")
summary(log.reg)
...
#Predict class
pred.class <- ifelse(predict(log.reg, type = "response") > 0.5, 1, 0)

#AUC
roc_log <- roc(data$chag, predict(log.reg, type = "response"))
auc(roc_log)
plot(roc_log)

#Regularization
set.seed(123)
x <- cbind(HnRNPA1_Serum = data$HnRNPA1_Serum, dummy = rnorm(nrow(data), mean = 0, sd = 1e-6))
x <- as.matrix(x)
