...
ggplot(combined_auc, aes(x = reorder(paste(Biomarker, Sample, sep = " "), AUC), y = AUC, fill = Category)) +
  ...
  scale_fill_manual(values = c("Excellent (≥ 0.9)" = "#1b9e77",
                               "Good (0.8–0.9)" = "#d95f02",
                               "Moderate/Low (< 0.8)" = "#7570b3")) +
  labs(title = "AUC Values for Biomarkers in Chagas Disease",
       subtitle = "Determining Diagnostic vs Prognostic Variables for Chagas Disease",
       x = "Biomarker",
       y = "AUC",
       fill = "AUC Category") +
...
ggplot(combined_auc_labeled, aes(x = reorder(paste(Biomarker, Sample, sep = " "), AUC), 
  ...
  coord_flip() +
  facet_wrap(~Group, scales = "free_y") +
  scale_fill_manual(values = c("Diagnostic" = "#1b9e77",
                               "Prognostic" = "#d95f02",
                               "Both" = "#7570b3")) +
  labs(title = "AUC Values for Biomarkers in Chagas Disease",
...
# Create a "disease" column for asym,sym (1) and NHS (0) 
mdata <- mdata %>%
...
# Create a "sym" column for asym (0) and sym (1)
mdata <- mdata %>%
